    ...
    return (
        <ListGroup style={{ width: '100%', height: '100%' }}>
            { items.map((item, index) =>
                <ListGroupItem style={{ height: 80, minHeight: 80, display: "flex", alignItems: "center", margin: 10, boxShadow: `${8}px ${8}px ${3}px black` }} className="justify-content-between" key={index}>
                    <div style={{ width: 150, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                        {item.name}
                        <FaTimes style={{ margin: 5 }} /> <Badge pill >{item.quantity} </Badge>
                    </div>
                    <div>
                        <h1 style={{ fontSize: 20 }}>Price:</h1>
                        <p>${item.price}</p>
                    </div>
                    <Button onClick={() => deleteItem(item.productId)} style={{ backgroundColor: "red", }}><FaTimes /> </Button>
                </ListGroupItem>
            )}
        </ListGroup>
    )
